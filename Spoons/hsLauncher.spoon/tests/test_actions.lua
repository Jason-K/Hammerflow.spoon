...
local function resetStateStorage()
    execCalls = {}
    windowCalls = {}
    assignHotkeyCalls = 0
...
local preloadFactories = {
    ...
    ['hsLauncher.main.core.logger'] = function()
        return {
            info = function() end,
            error = function() end,
        }
    end,
    ['hsLauncher.main.core.action_runner'] = function()
        return {
            exec = function(spec)
    ...
    ['hsLauncher.main.core.windows_native'] = function()
        return setmetatable({}, {
    ...
    ['hsLauncher.main.modules.hotkeys.assign_hotkey'] = function()
        return {
            assign = function()
    ...
    ['hsLauncher.main.modules.hotkeys.assign_global'] = function()
        return {
    ...
    ['hsLauncher.main.core.hyper_modal'] = function()
        return { exitMode = function() end }
    end,
    ['hsLauncher.main.core.window_history'] = function()
        return {}
    end,
    ['hsLauncher.main.core.window_geometry'] = function()
        return {}
    end,
    ['hsLauncher.main.core.window_neighbors'] = function()
        return {}
    end,
    ['hsLauncher.main.core.modal_gui'] = function()
        return { show = function() end, hide = function() end }
    end,
    ['hsLauncher.main.core.config'] = function()
        return {}
    end,
}

for name, factory in pairs(preloadFactories) do
...
local function resetState()
    resetStateStorage()
    if Actions.clearLastAction then
        Actions.clearLastAction()
    end
end
...
local function assertEquals(actual, expected, message)
    if actual ~= expected then
        error((message or "assertEquals failed") ..
...
addTest('Actions.redoLast replays most recent action', function()
    resetState()
    local first = Actions.resolve(Actions.keystroke('g', { 'cmd' }))
    assert(first, 'Expected keystroke action to resolve')
    first()
    assertEquals(#execCalls, 1)
    local redo = Actions.resolve(Actions.redoLast())
    assert(redo, 'Expected redo action to resolve')
    redo()
    assertEquals(#execCalls, 2)
    assertEquals(execCalls[2].key, 'g')
    redo()
    assertEquals(#execCalls, 3)
    assertEquals(execCalls[3].key, 'g')
end)

addTest('Actions.redoLast without history is a no-op', function()
    resetState()
    local redo = Actions.resolve(Actions.redoLast())
    assert(redo, 'Expected redo action to resolve')
    redo()
