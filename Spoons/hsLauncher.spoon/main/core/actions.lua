...
local function wrapSafe(fn, label, actionMeta)
    local runner
    runner = function(...)
        ...
        if not (actionMeta and actionMeta.skipHistory) then
            lastAction = {
                runner = runner,
...
register('redo', function(_)
    return function()
        local previous = lastAction
        if not previous or type(previous.runner) ~= 'function' then
            logger.info('actions: redo requested but no previous action')
            return
        end
        previous.runner()
    end
...
function Actions.resolve(action)
    ...
    if type(action) == 'table' then
        ...
        if handler then
            ...
            if fn then return wrapSafe(fn, kind .. ' action', action) end
        end
    end
    local extra = ''
    if type(action) == 'table' then
...
function Actions.clearLastAction()
    lastAction = nil
end

function Actions.redoLast()
    return { kind = 'redo', skipHistory = true }
end

function Actions.getLastActionLabel()
    return lastAction and lastAction.label or nil
end
