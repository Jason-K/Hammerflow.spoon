...
local function compileActionSpec(action)
    ...
    for _, step in ipairs(action.actions or {}) do
        ...
        elseif stepType == 'string' then
            logError(string.format('user.registry: action %s uses unsupported string step - convert to descriptor table',
                action.name))
        end
    end
...
local function buildActionEntryFromMenuItem(item, section)
    ...
    if not key or key == '' then
        logWarn(string.format('user.registry: menu %s action %s missing shortcut', tostring(item.menuId or 'root'),
            tostring(item.id)))
        return nil
    end
...
local function buildLegacyActionEntry(legacy, actionId, entry, defaultSection, moduleId)
    ...
    if not ok then
        logWarn(string.format('user.registry: legacy action %s failed to resolve -> %s', tostring(actionId),
            tostring(spec)))
        return nil
    end
    if spec == nil then
    ...
    local shortcut = normalizeShortcut(
    { shortcut = entry.key, defaultShortcut = entry.defaultShortcut, fallbackShortcut = entry.fallbackShortcut }, label)
...
function Registry.load(configOverride)
    ...
    state.features.menuBuilder = isFeatureEnabled(config, 'menuBuilder', 'HSLAUNCHER_MENU_BUILDER',
        state.features.menuBuilder or false)
    state.features.declarativeRuntime = true
